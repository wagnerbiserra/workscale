...
presencial = planejado = planejado_vencido = home = ferias = banco = atestado = dayoff = folga_plantao = 0
...
dias_validos = max(uteis - (total_nao_uteis + ferias + banco + atestado + dayoff), 0)
...
c13.metric("🚑 Atestado", atestado)
c14.metric("🎂 Day Off", dayoff)
c15.metric("🔄 Plantão", folga_plantao)
c16.metric("📌 Faltam", max(restante, 0))
...
def calcular_status(eventos, ano, mes, uteis, feriados_uteis, emendas_uteis):
    ...
    home = ferias = banco = atestado = dayoff = 0
...
if user.get("tipo") == "gestor":
    ...
    for membro in equipe:
        ...
        pres = plan = home = ferias = banco = atestado = folga_plantao = 0
